...
const logoModel = require('../data/models/logo.model.server');

const TEAMIDS_ENGLISH = ['40', '50', '47', '42', '49', '33', '46', '39',
                                 '45', '38', '48', '35', '52', '44', '34', '41', '51', '43', '36', '37'];

const TEAMIDS_FRENCH = ['85', '79', '80', '1063', '81', '82', '93', '84', '95', '92',
                                '94', '77', '78', '83', '96', '91', '87', '89', '90', '88'];

const TEAMIDS_GERMAN = ['157', '165', '173', '169', '163', '167', '162', '168', '161',
                                '158', '159', '164', '160', '170', '174', '172', '171', '166'];

const TEAMIDS_SPANISH = ['529', '530', '541', '536', '546', '532', '531', '542', '543',
                                 '548', '537', '540', '545', '547', '533', '539', '720', '538', '728', '726'];

const TEAMIDS_ITALIAN = ['496', '492', '505', '489', '497', '499', '487', '503', '498',
                                 '502', '488', '490', '493', '523', '495', '494', '500', '511', '512', '491'];

const TEAMIDS_ALL = ['40', '50', '47', '42', '49', '33', '46', '39',
                             '45', '38', '48', '35', '52', '44', '34', '41', '51', '43', '36', '37',
...
truncateLogos = () => {
  ...
  logoModel.countDocuments().then(
    response => {

      // Only drop if it has any records
      if (response !== 0) {
        mongoose.connection.collections['logo'].drop();
      }
    }
  );
};
...
populateLogos = () => {
  ...
  for (let key in TEAM_LOGO_URLS) {

    let url = TEAM_LOGO_URLS[key];
    ...
    logoModel.create(teamLogo);
  }

  for (let key in LEAGUE_LOGO_URLS) {

    let url = LEAGUE_LOGO_URLS[key];
    ...
    logoModel.create(leagueLogo);
  }

};

findLogoUrlByTeam = tid =>
  logoModel.find({teamId: tid});

findLogoUrlByLeague = lid =>
  logoModel.find({leagueId: lid});
